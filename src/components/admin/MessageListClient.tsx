  ...
  return (
    <div className="space-y-4">
      {initialMessages.map((msg) => (
        <div 
          key={msg.id} 
          className={`glass-panel overflow-hidden transition-all duration-300 ${!msg.isRead ? 'border-l-4 border-l-neon' : 'border-l-4 border-l-transparent'}`}
        >
          {/* Header Row */}
          <div 
            className={`p-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4 cursor-pointer hover:bg-white/5 transition-colors ${!msg.isRead ? 'bg-neon/5' : ''}`}
            onClick={() => {
          ...
          >
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-3 mb-1">
                <span className={`font-bold truncate ${!msg.isRead ? 'text-neon' : 'text-slate-300'}`}>
                  {msg.name}
                </span>
                {msg.isReplied && (
              ...
              <div className="text-xs text-slate-500 truncate">
                <span className="text-neon/50 mr-2">{msg.email}</span>
          ...
          {expandedId === msg.id && (
            <div className="p-5 border-t border-white/10 bg-black/40">
              <div className="mb-6">
                ...
                <p className="text-sm text-slate-300 leading-relaxed whitespace-pre-wrap bg-[#050505]/80 p-4 border border-neon/20 rounded-sm custom-scrollbar">
                  {msg.message}
                </p>
              </div>
              ...
              {/* Inline Reply Form */}
              <div className="border-t border-neon/20 pt-5">
                <span className="text-[9px] text-neon font-bold tracking-widest uppercase mb-3 block flex items-center gap-2">
                ...
                <form onSubmit={(e) => handleReplySubmit(e, msg.id, msg.email, msg.name)} className="space-y-3">
                  <textarea
                    ...
                    required
                    className="w-full bg-neon/5 border border-neon/30 p-3 text-sm text-white focus:outline-none focus:border-neon focus:bg-neon/10 transition-colors custom-scrollbar"
                  />
                  <button 
                    type="submit" 
                    disabled={replyStatus === 'sending'}
                    className="bg-neon/10 text-neon border border-neon/50 px-6 py-2 hover:bg-neon/20 text-xs font-bold uppercase tracking-widest transition-all disabled:opacity-50"
                  >
