...
export default function FloatingDock() {
  ...
  return (
    <div className="fixed bottom-6 left-1/2 -translate-x-1/2 z-50 flex justify-center w-full max-w-fit px-4 pointer-events-none">
      <div className="glass-panel px-4 py-3 rounded-2xl flex items-center gap-4 bg-black/60 backdrop-blur-lg border border-white/10 shadow-[0_0_30px_rgba(0,0,0,0.8)] pointer-events-auto">
        {dockItems.map((item, idx) => (
          ...
          >
            ...
            <motion.a
              ...
              whileTap={{ scale: 0.95 }}
              className="flex items-center justify-center w-11 h-11 rounded-full bg-white/5 border border-white/10 text-gray-400 hover:text-cyan-400 transition-colors cursor-pointer"
            >
              {item.icon}
            </motion.a>
          </div>
