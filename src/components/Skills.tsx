...
// Mapping of custom SVG icons for each skill (used for both the badge and the large HUD watermark)
...
interface Skill {
  ...
  status: string
  desc: string
  logs: string[] // Compilation/run outputs
}

const skillsData: Skill[] = [
  { name: 'TypeScript', category: 'Languages', color: 'hover:text-[#3178C6] hover:border-[#3178C6]/50 hover:bg-[#3178C6]/5', level: '92%', latency: '8ms', status: 'READY', desc: 'Type-safe JavaScript compiler layers', logs: ['tsc --watch', 'parsing tsconfig.json...', 'compiled build successfully', '0 semantic errors found.'] },
  { name: 'JavaScript', category: 'Languages', color: 'hover:text-[#F7DF1E] hover:border-[#F7DF1E]/50 hover:bg-[#F7DF1E]/5', level: '95%', latency: '6ms', status: 'READY', desc: 'ECMAScript core engine & event loops', logs: ['node server.js', 'V8 engine initialized', 'processing event queue...', 'runtime check: OK'] },
  { name: 'Python', category: 'Languages', color: 'hover:text-[#3776AB] hover:border-[#3776AB]/50 hover:bg-[#3776AB]/5', level: '85%', latency: '24ms', status: 'OPTIMIZED', desc: 'Automation scripting & data science schemas', logs: ['python script.py', 'import sys, pandas, os', 'data array structured', 'compiling model: 100%'] },
  { name: 'Dart', category: 'Languages', color: 'hover:text-[#0175C2] hover:border-[#0175C2]/50 hover:bg-[#0175C2]/5', level: '78%', latency: '18ms', status: 'READY', desc: 'Strongly typed application code', logs: ['dart run bin/main.dart', 'initializing VM resolver', 'AOT compilation checks pass', 'hot reload daemon active'] },
  { name: 'C++', category: 'Languages', color: 'hover:text-[#00599C] hover:border-[#00599C]/50 hover:bg-[#00599C]/5', level: '80%', latency: '4ms', status: 'OPTIMIZED', desc: 'High performance native compilation', logs: ['g++ main.cpp -o main', 'linker map structured', 'binary execution initialized', 'memory allocation layout: stable'] },
  { name: 'Java', category: 'Languages', color: 'hover:text-[#ED8B00] hover:border-[#ED8B00]/50 hover:bg-[#ED8B00]/5', level: '75%', latency: '35ms', status: 'READY', desc: 'OOP systems & enterprise integrations', logs: ['javac Main.java && java Main', 'class loaders mapped', 'garbage collector active', 'thread scheduling stable'] },
  
  { name: 'Next.js', category: 'Frontend', color: 'hover:text-white hover:border-white/50 hover:bg-white/5', level: '90%', latency: '12ms', status: 'OPTIMIZED', desc: 'Server components & hydration routes', logs: ['next build', 'collecting page pathways...', 'generating static files (9/9)', 'turbopack server listening...'] },
  { name: 'React', category: 'Frontend', color: 'hover:text-[#61DAFB] hover:border-[#61DAFB]/50 hover:bg-[#61DAFB]/5', level: '94%', latency: '8ms', status: 'READY', desc: 'Virtual DOM structures & hooks telemetry', logs: ['react-dom: hydration initialized', 'fiber tree reconciled in 0.4ms', '0 warnings in dynamic rendering', 'hooks bindings stable'] },
  { name: 'Tailwind CSS', category: 'Frontend', color: 'hover:text-[#06B6D4] hover:border-[#06B6D4]/50 hover:bg-[#06B6D4]/5', level: '96%', latency: '2ms', status: 'READY', desc: 'Declarative layout architectures', logs: ['tailwindcss --watch', 'building index.css...', 'utility map constructed in 28ms', 'purging unused directives'] },
  { name: 'Framer Motion', category: 'Frontend', color: 'hover:text-[#F024B6] hover:border-[#F024B6]/50 hover:bg-[#F024B6]/5', level: '88%', latency: '15ms', status: 'READY', desc: 'High-performance UI spring physics', logs: ['framer-motion: register springs', 'transform maps aligned', 'layout animations enabled', 'hardware acceleration engaged'] },
  { name: 'Flutter', category: 'Frontend', color: 'hover:text-[#02569B] hover:border-[#02569B]/50 hover:bg-[#02569B]/5', level: '80%', latency: '22ms', status: 'READY', desc: 'Multi-platform engine view layouts', logs: ['flutter run', 'skia graphic engine initialized', 'rendering 60FPS UI threads', 'debug handshake verified'] },
  
  { name: 'Node.js', category: 'Backend', color: 'hover:text-[#339933] hover:border-[#339933]/50 hover:bg-[#339933]/5', level: '88%', latency: '14ms', status: 'READY', desc: 'Non-blocking I/O event structures', logs: ['node server.js', 'attaching process port listeners', 'event loops starting...', 'memory footprint: 28MB'] },
  { name: 'Express', category: 'Backend', color: 'hover:text-gray-300 hover:border-gray-500/50 hover:bg-gray-500/5', level: '88%', latency: '10ms', status: 'READY', desc: 'REST request handling layer', logs: ['express: load sub-routes', 'CORS configurations parsed', 'registered request handlers', 'standing by for payloads'] },
  { name: 'PostgreSQL', category: 'Backend', color: 'hover:text-[#4169E1] hover:border-[#4169E1]/50 hover:bg-[#4169E1]/5', level: '84%', latency: '28ms', status: 'OPTIMIZED', desc: 'Relational data indexing schemas', logs: ['pg_ctl start', 'database system is ready', 'active connections: 4/100', 'shared buffers caching complete'] },
  { name: 'Prisma', category: 'Backend', color: 'hover:text-white hover:border-white/50 hover:bg-white/5', level: '90%', latency: '5ms', status: 'READY', desc: 'Type-safe object relational map queries', logs: ['prisma generate', 'reading schema.prisma...', 'constructing client model mappings', 'client generated successfully'] },
  { name: 'Supabase', category: 'Backend', color: 'hover:text-[#3ECF8E] hover:border-[#3ECF8E]/50 hover:bg-[#3ECF8E]/5', level: '85%', latency: '16ms', status: 'READY', desc: 'PostgREST api structures & auth services', logs: ['supabase start', 'local containers started', 'listening on localhost:54321', 'realtime socket listener: active'] },
  { name: 'REST APIs', category: 'Backend', color: 'hover:text-cyan-400 hover:border-cyan-500/50 hover:bg-cyan-500/5', level: '92%', latency: '12ms', status: 'READY', desc: 'HTTP data-exchange protocols', logs: ['fetching endpoints mapping...', 'parsing payload body JSONs', '200 OK - data parsed successfully', 'standing by for new packets'] },
  
  { name: 'Git', category: 'Tools_&_DevOps', color: 'hover:text-[#F05032] hover:border-[#F05032]/50 hover:bg-[#F05032]/5', level: '90%', latency: '4ms', status: 'READY', desc: 'Decentralized source controls', logs: ['git status', 'on branch main', 'your branch is up to date', 'nothing to commit, working tree clean'] },
  { name: 'GitHub', category: 'Tools_&_DevOps', color: 'hover:text-white hover:border-white/50 hover:bg-white/5', level: '92%', latency: '5ms', status: 'READY', desc: 'Action automations & repositories', logs: ['gh repo sync', 'checking workflows status...', 'CI actions parsed: SUCCESS', 'pull requests verified'] },
  { name: 'Docker', category: 'Tools_&_DevOps', color: 'hover:text-[#2496ED] hover:border-[#2496ED]/50 hover:bg-[#2496ED]/5', level: '80%', latency: '45ms', status: 'OPTIMIZED', desc: 'Container sandbox virtualization', logs: ['docker-compose build', 'constructing layer images...', 'sandbox container isolated', 'containers listening on dev bridges'] },
  { name: 'Vercel', category: 'Tools_&_DevOps', color: 'hover:text-white hover:border-white/50 hover:bg-white/5', level: '95%', latency: '8ms', status: 'READY', desc: 'Edge routing & static CDN layouts', logs: ['vercel deploy', 'inspecting project contents...', 'production URL generated', 'edge CDN caches built'] },
  { name: 'Linux', category: 'Tools_&_DevOps', color: 'hover:text-[#FCC624] hover:border-[#FCC624]/50 hover:bg-[#FCC624]/5', level: '85%', latency: '10ms', status: 'READY', desc: 'Bash scripts & process handlers', logs: ['bash script.sh', 'verifying shell directives...', 'environment maps parsed', 'process daemon background initialized'] },
  { name: 'Postman', category: 'Tools_&_DevOps', color: 'hover:text-[#FF6C37] hover:border-[#FF6C37]/50 hover:bg-[#FF6C37]/5', level: '88%', latency: '12ms', status: 'READY', desc: 'API route sandboxes & diagnostics', logs: ['postman run collection.json', 'checking HTTP methods status', 'testing environment variables', 'all routes verified: SUCCESS'] }
]
...
export default function Skills() {
  ...
  // Real-time ticking system stats for that premium "alive" HUD feeling
  const [systemLoad, setSystemLoad] = useState({ cpu: '21.4%', mem: '142MB' })

  // Fluctuate CPU and Memory stats every 1.5 seconds to simulate system activity
  useEffect(() => {
    const interval = setInterval(() => {
      const cpuVal = (15 + Math.random() * 20).toFixed(1) + '%'
      const memVal = Math.floor(130 + Math.random() * 30) + 'MB'
      setSystemLoad({ cpu: cpuVal, mem: memVal })
    }, 1500)
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 items-stretch">
        ...
        <div className="lg:col-span-1">
          <FadeIn delay={0.25} direction="up">
            <div className="relative glass-panel p-6 border border-white/10 rounded-lg bg-black/85 shadow-[0_0_40px_rgba(0,240,255,0.02)] h-full min-h-[380px] flex flex-col justify-between overflow-hidden">
              
              {/* Dynamic Watermark Background Logo of hovered skill */}
              <div className="absolute inset-0 flex items-center justify-center pointer-events-none select-none z-0 overflow-hidden">
                <AnimatePresence>
                  {hoveredSkill && skillIcons[hoveredSkill.name] && (
                    ...
                    >
                      {skillIcons[hoveredSkill.name]}
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>

              {/* HUD Coordinates / Hardware Stats */}
              <div className="absolute top-2 right-2 text-[8px] text-gray-600 font-mono tracking-wider z-10 flex gap-2">
                <span>CPU: <span className="text-cyan-400">{systemLoad.cpu}</span></span>
                <span>MEM: <span className="text-cyan-400">{systemLoad.mem}</span></span>
              </div>
              ...
              <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div>
                  <div className="flex items-center gap-2 border-b border-white/10 pb-3 mb-4">
                    ...
                    <span className="text-[10px] uppercase tracking-wider text-cyan-400 font-bold">SYSTEM_DIAGNOSTICS v3.5</span>
                  </div>
                  ...
                  <AnimatePresence mode="wait">
                    {hoveredSkill ? (
                      <motion.div
                        ...
                        className="space-y-3.5 text-xs text-gray-400 font-mono"
                      >
                        {/* Title block */}
                        <div>
                          <span className="text-gray-600 text-[10px] font-bold">// TARGETNAME</span>
                          <div className="text-sm font-extrabold text-white tracking-widest uppercase flex items-center gap-1.5">
                            {hoveredSkill.name}
                          </div>
                        </div>

                        {/* Telemetry info layout */}
                        <div className="grid grid-cols-2 gap-2 border-y border-white/5 py-2.5">
                          <div>
                            <span className="text-gray-600 text-[9px] font-bold">// DOMAIN</span>
                            <div className="text-cyan-300 font-semibold">{hoveredSkill.category}</div>
                          </div>
                          <div>
                            <span className="text-gray-600 text-[9px] font-bold">// DELAY_TIME</span>
                            <div className="text-green-400 font-semibold">{hoveredSkill.latency}</div>
                          </div>
                        </div>

                        {/* Interactive Level percentage slider */}
                        <div className="space-y-1">
                          <div className="flex justify-between items-center text-[9px]">
                            <span className="text-gray-600 font-bold">// INTEGRATION_RATIO</span>
                            <span className="text-cyan-400 font-bold">{hoveredSkill.level}</span>
                          </div>
                          <div className="flex-1 bg-white/5 h-2 rounded-sm overflow-hidden border border-white/10 p-[1px]">
                            <motion.div
                              initial={{ width: 0 }}
                        ...
                        {/* Summary description */}
                        <div>
                          <span className="text-gray-600 text-[9px] font-bold">// COMPILER_SCOPE</span>
                          <p className="text-[10px] leading-relaxed text-gray-300 font-sans mt-0.5">
                            {hoveredSkill.desc}
                          </p>
                        </div>

                        {/* Terminal compilation logs (Option 3 Dynamic add-on) */}
                        <div className="bg-black/50 border border-white/5 p-2.5 rounded font-mono text-[9px] leading-tight space-y-1 text-gray-400">
                          <div className="text-[8px] text-gray-600 font-bold tracking-widest uppercase mb-1">
                            // RUNTIME_COMPILER_LOGS
                          </div>
                          {hoveredSkill.logs.map((logLine, idx) => (
                            <motion.div 
                              key={idx}
                              initial={{ opacity: 0, x: -5 }}
                              animate={{ opacity: 1, x: 0 }}
                              transition={{ delay: idx * 0.1 }}
                              className={idx === 0 ? 'text-cyan-400 font-semibold' : 'text-gray-500'}
                            >
                              {idx === 0 ? '> ' : '  '}{logLine}
                            </motion.div>
                          ))}
                      ...
                      <motion.div
                        ...
                        animate={{ opacity: 0.5 }}
                        exit={{ opacity: 0 }}
                        className="h-48 flex flex-col items-center justify-center text-center font-mono text-[9px] text-gray-500 space-y-3 py-6"
                      >
                        <svg className="w-8 h-8 animate-pulse text-cyan-500/40" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="1.5" d="M9.75 17L9 20l-1 1h8l-1-1-.75-3M3 13h18M5 17h14a2 2 0 002-2V5a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                        </svg>
                        <div className="uppercase tracking-widest font-bold text-cyan-500/40 animate-pulse">
                          [ HUD_STANDBY_MODE ]
                        </div>
                        <div className="max-w-[200px] leading-normal uppercase">
                          HOVER OVER ANY COMPILATION SKILL CARD TO FEED REAL-TIME telemetry DATA TO THE DECKS.
                        </div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                ...
                <div className="border-t border-white/5 pt-2 mt-4 text-[8px] text-gray-600 flex justify-between select-none">
                  <span>SECTOR: 0xFF02</span>
                  <span>LOAD_telemetry: STABLE</span>
                </div>
