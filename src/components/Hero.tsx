  ...
  return (
    <section className="relative flex flex-col items-center justify-center w-full min-h-screen bg-transparent text-white m-0 p-0 selection:bg-[#00f0ff] selection:text-black font-sans">
      ...
      <div className="relative z-10 w-full max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-8 items-center">        <motion.div variants={containerVariants} initial="hidden" animate="visible" className="lg:col-span-7 flex flex-col items-start text-left">
        ...
        <motion.h1 variants={itemVariants} className="font-mono font-black text-5xl sm:text-6xl md:text-7xl lg:text-[6rem] leading-[0.9] text-transparent bg-clip-text bg-gradient-to-r from-white via-slate-200 to-slate-500 uppercase tracking-tighter mb-2" style={{ textShadow: '0px 0px 20px rgba(255,255,255,0.1)' }}>
              ...
              <AnimatePresence initial={false}>
                {history.map((line) => (
                  <motion.div key={line.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="mb-1.5">
                    ...
                    {line.type === 'input' && <div className="flex items-center gap-2 text-[#00f0ff]"><span className="font-bold">root@sys:~#</span><span>{line.text}</span></div>}
                    {line.type === 'output' && <div className={`pl-2 ${line.text.includes('[OK]') || line.text.includes('exit code') ? 'text-emerald-400' : line.text.startsWith('>') ? 'text-[#00f0ff] brightness-110' : 'text-slate-300'}`}>{line.text}</div>}
                  </motion.div>
                ))}
              </AnimatePresence>
