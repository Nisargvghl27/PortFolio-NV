    ...
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 font-mono">
      {leadershipData.map((item, index) => (
        <FadeIn key={item.id} delay={0.1 + index * 0.1} direction="up">
          <div className="relative group bg-white/[0.02] hover:bg-white/[0.04] border border-white/5 hover:border-cyan-500/30 p-6 h-full flex flex-col justify-between rounded-sm transition-all duration-300 shadow-[0_4px_20px_rgba(0,0,0,0.15)]">
            ...
            {/* Description */}
            <p className="text-xs text-gray-400 leading-relaxed font-light mt-auto">
              {item.desc}
            </p>
